...
import { useGetWalletBalanceQuery } from '@silicoin/api-react';
  ...
  */
  ...
  const points = [
    {
      ...
      y: Math.max(0, mojo_to_silicoin(start)),
      tooltip: mojo_to_silicoin(balance),
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: Math.max(0, mojo_to_silicoin(start)),
      tooltip: mojo_to_silicoin(start),
    });
  });
