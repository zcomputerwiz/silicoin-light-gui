...
import { Card, CardKeyValue, CopyToClipboard, Flex, Loading, StateColor, TableControlled } from '@silicoin/core';
...
const getCols = (type: WalletType) => [
      ...
      return (
        <>
          ...
          <strong>
            ...
            : mojo_to_silicoin_string(row.amount)}
          </strong>
          &nbsp;
          {metadata.unit}
        </>
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>{mojo_to_silicoin_string(row.feeAmount)}</strong>
        &nbsp;
        {metadata.feeUnit}
      </>
    ),
