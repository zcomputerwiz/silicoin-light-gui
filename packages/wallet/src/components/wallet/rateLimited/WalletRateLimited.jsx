...
import { AlertDialog, Card, Flex } from '@silicoin/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const silicoinper_input = ip_parsed.limit;
    ...
    const silicoinper_value = Number.parseInt(Number(silicoinper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (silicoin per interval):{' '}
                  {mojo_to_silicoin_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = silicoin_to_mojo(fee_input.value);
