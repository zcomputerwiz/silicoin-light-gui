...
const PY_FOLDER = '../../../silicoin/daemon';
...
const getSilicoinVersion = () => {
  let version = null;
  const exePath = getExecutablePath('silicoin');
  // first see if we can get a silicoin exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or silicoin is on the path
    try {
      version = child_process
