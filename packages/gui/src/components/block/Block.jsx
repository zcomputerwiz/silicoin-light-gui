...
export default function Block() {
  ...
  const poolReward = mojo_to_silicoin(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_silicoin(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const silicoinFees = blockRecord.fees
    ? mojo_to_silicoin(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.silicoinexplorer.com/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.silicoinexplorer.com/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: silicoinFees ? `${silicoinFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the Silicoin blockchain
            </Trans>
          </Back>
        }
        action={
