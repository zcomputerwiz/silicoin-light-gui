...
import { AlertDialog } from '@silicoin/core';
import {
  Typography,
  Button,
  Box,
...
import { silicoin_to_mojo } from '../../../util/silicoin';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      silicoinper_input.value === '' ||
      Number(silicoinper_input.value) === 0 ||
      !Number(silicoinper_input.value) ||
      isNaN(Number(silicoinper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const silicoinper = silicoin_to_mojo(silicoinper_input.value);
    const silicoinper_value = Number.parseInt(Number(silicoinper));
    const userpubkey = userpubkey_input.value;
    const amount = silicoin_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = silicoin_to_mojo(fee_input.value);
